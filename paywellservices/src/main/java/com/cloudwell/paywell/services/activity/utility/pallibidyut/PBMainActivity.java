...
public class PBMainActivity extends AppCompatActivity {
    ...
    private static final String POLLY_BIDDUT = "Polly Biddut";
    boolean hasPBService = false;
    ...
    private void initView() {
        ...
        for (int i = 0; i < serviceList.size(); i++) {
            if (POLLY_BIDDUT.equalsIgnoreCase(serviceList.get(i))) {
                hasPBService = true;
                break;
            }
        }
    ...
    public void onButtonClicker(View v) {
        ...
        switch (v.getId()) {
            ...
            case R.id.homeBtnInquiryReg:
                if (hasPBService) {
                    startActivity(new Intent(this, PBInquiryRegActivity.class));
                    finish();
                } else {
                    Snackbar snackbar = Snackbar.make(mRelativeLayout, R.string.services_off_msg, Snackbar.LENGTH_LONG);
                    snackbar.setActionTextColor(Color.parseColor("#ffffff"));
            ...
            case R.id.homeBtnInquiryBillPay:
                if (hasPBService) {
                    startActivity(new Intent(this, PBInquiryBillPayActivity.class));
                    finish();
                } else {
                    Snackbar snackbar = Snackbar.make(mRelativeLayout, R.string.services_off_msg, Snackbar.LENGTH_LONG);
                    snackbar.setActionTextColor(Color.parseColor("#ffffff"));
